...
def main() -> None:
    ...
    def multi_query_retriever(question, index, active_provider, top_k):
        results, traces = pipeline.retrieve(question, top_k)
        rerank_traces.extend(traces)
        return results
    ...
    pipeline = "multi-query-rrf-llm-reranker"
    run_config = {
        "run_at_utc": datetime.now(timezone.utc).isoformat(),
        "pipeline": pipeline,
        "decomposition": "explicit coordinated-clause split",
        "merge": "round-robin unique full-question then per-clause rankings",
    ...
    print("\nMulti-query hybrid retrieval results")
